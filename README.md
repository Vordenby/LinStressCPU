...
- Python 3.8+
- Рекомендуется установить `psutil` для управления приоритетами процесса на разных ОС:
	```bash
	pip install psutil
	```

## Installation / Установка
Клонируйте репозиторий и установите опциональные зависимости:

```bash
git clone <repo-url>
cd LinStressCPU
pip install -r requirements.txt  # опционально, содержит psutil
```
...
## Packaging / Упаковка
Можно упаковать CLI или GUI в исполняемый файл через `PyInstaller`:

```bash
pip install pyinstaller
pyinstaller --onefile LinStress.py
pyinstaller --onefile LinStressGUI.py
```
